...
const SmallProjectCard = ({project, idx, setSelectedProject}: {project: SmallProject, idx: number, setSelectedProject: (id: string) => void}) => {
    return (
        <motion.div key={project.id} initial={{opacity: 0, scale: 0.9}} whileInView={{opacity: 1, scale: 1}} viewport={{once: true}} transition={{delay: idx * 0.05}} whileHover={{scale: 1.05, y: -5}} className="cursor-pointer">
            <div className="rounded-2xl shadow-[8px_8px_16px_#bebebe,-8px_-8px_16px_#ffffff] hover:shadow-[12px_12px_24px_#bebebe,-12px_-12px_24px_#ffffff] transition-shadow p-6">
                {project.embedUrl ? (
